...
if(isset($_POST['log_btn'])){
	...
	$check = mysqli_query($link, "SELECT * FROM regdetails WHERE email='$em' AND pass='$password'");
	$row = mysqli_num_rows($check);

	if($row == 1) {
        header ("Location: dashboardemp.php");
        $uname= mysqli_query($link, "SELECT username FROM regdetails WHERE email='$em' AND pass='$password'");
		...
		echo "<h5><br> <br>Email and Password does not match &#128531</h5>";
	}
}
